...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   - 20 байт
   - 56(total lenght) - 20(header lenght) = 36
...
<img src="images/question1part2.png" width=600/>
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - не всегда, но изменяется TTL (ожидаемо)
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Destination Address -- потому что это и есть адрес, путь до которого мы пытаемся проследить, Total Length -- потому что мы ее задали, как 56
      - TTL (каждые n пакетов, в моем случае 3), identification и следовательно header checksum
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - увеличивается на 1 по сравнению с предыдущим
      - еще у первой дейтаграммы identification == src port + 1, т.е. identification == src port + номер запроса
6. Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL
   (выберите какой-либо один пакет ICMP)?
   - 0
   - 254
  
<img src="images/question1part3.png" width=600/>
...
7. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - если все сообщения, то почти у всех Identification 0, но не у всех; TTL спокойно колеблется около 240-250
   - в рамках сообщений, которые поступили на компьютер с
   ближайшего (первого транзитного) маршрутизатора -- остаются неизменными.
9. Найдите серии откликов ICMP, в которых содержатся сообщения об истечении
   ...
   - 245
     ...
     <img src="images/question1part4.png" width=600/>
     
10. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
...
<img src="images/question1part5.png" width=600/>

   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      Если да, то сколько фрагментов было создано?
      - поступившее обратно ICMP-сообщение с time-to-live exceeded не фрагментировано, просто содержит последний udp-фрагмент из тех, что были отправлены. (А исходное -- да, 3 фрагмента) (см скрин)
