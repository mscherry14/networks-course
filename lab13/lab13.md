...
Комментарий: мак не умеет делать `ipconfig /release` и `ipconfig /renew`. Но у меня получилось поймать момент запроса IP "в дикой природе".
...
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   ...
   - 0x3c2a82c5
   - идентификатор транзакции, отличает процесс получения IP-адреса от других, протекающих в то же время. 
...
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   ...
   - Количество IP-адресов в сети ограничено, а количество устройств, которые хочется подключить к сети за все время ее существования как правило гораздо больше, чем количество IP-адресов. => Хочется освобождать "неиспользуемые" IP-адреса, чтобы давать их новым подключающимся устройствам + все устройства должны точно знать свой IP или не знать вообще (то есть, переложить ответсвенность на раздачу IP с устройств на DHCP-сервер не только бы было неэффективным и противоречило бы идее "выносить сложность на периферию", но еще  уязвимым к неконсистентным состояниям, где сервер поменял IP для старого устройства и выдал его новому, но не успел сообщить об этом старому/сообщение не дошло, и оказалось два устройства с одним IP. То есть, ТОЧНО НЕ ВАРИАНТ). И...
   - В общем, если кратко, то для того, чтобы неиспользумые IP-адреса освобождались и сеть продолжала работать, назначать срок аренды оказалось лучшим/самым жизнеспособным/просто жизнеспособным способом это реализовать
...
<img src="images/question2part1.png" />

<img src="images/question2part2.png" />
