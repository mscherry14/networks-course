...
1. Выберите один UDP-пакет. По этому пакету определите, сколько полей содержит UDP-заголовок.
   - 4 поля -- Source Port, Destination Port, Length, Cheksum
...
3. Значение в поле Length (Длина) – это длина чего?
   - Длина UDP пакета в байтах -- заголовок 8 байт + данные
4. Какое максимальное количество байт может быть включено в полезную нагрузку UDP-пакета?
   - 65535 - 60 (IP) - 8 (UDP), без учета фрагментации
   - MTU — IP — UDP = 1500 — 60 — 8 = 1432 байт, если следует избегать фрагментации (размер служебной информации IP-протокола -- от 20 до 60 байт в зависимости от опций)
...
6. Какой номер протокола для протокола UDP? Дайте ответ и для шестнадцатеричной и
   ...
   - В десятеричной: 17
   - В шестандцатеричной: 11
7. Проверьте UDP-пакет и ответный UDP-пакет, отправляемый вашим хостом. Определите
   отношение между номерами портов в двух пакетах.
   - Они меняются местами (ответ приходит с того порта, на который делался запрос и тому порту, с которого делался запрос)
